...
public class MemoryFileRepository implements MemoryRepository {
    ...
    public Memory add(Memory memory) throws DataAccessException {
        // 1. Get all existing memories.
        List<Memory> all = findAll();
        // 2. Determine the next unique identifier.
        int nextId = getNextId(all);
        // 3. Set the new memory's id.
        memory.setId(nextId);
        // 4. Add the new memory to the existing memories.
        all.add(memory);
        // 5. Write all memories to the data file.
        writeToFile(all);
        // 6. Return the memory with a brand-new id.
        return memory;
    }
